  ...
  return <main>
    ...
    <div className="shell" id="control"><aside><p className="asideLabel">Pipeline</p><a className="selected" href="#control">Control tower</a><a href="#quotes">Quote inbox <span>{quotes.length}</span></a><a href="#intake">Intake</a><a href="#consolidation">Pallet planning</a><a href="#orders">Orders <span>{snapshot?.orders_open ?? 0}</span></a><a href="#tracking">Customer tracking</a><p className="asideLabel second">Authority</p><div className="facility"><small>System of record</small><strong>Supabase FEP</strong><span>Every mutation returns a receipt</span></div></aside>
      <div className="content"><div className="pageTitle"><div><p className="eyebrow">Live operational data</p><h1>Operations control tower</h1><p>Order intake, customs readiness, consolidation, handoffs, and customer tracking.</p></div><div className="scope"><span>Network</span><strong>Türkiye + Italy → USA</strong></div></div>
        ...
        <section className="panel quoteInbox" id="quotes"><div className="panelHeader"><div><p className="eyebrow">Customer → operations handoff</p><h2>Quote inbox</h2><p className="muted">Build the verified provider cost chain, apply the volume-based Gel Öz fee, and publish a customer-ready firm offer.</p></div><a href="/" target="_blank">Open customer calculator ↗</a></div><div className="quoteInboxRows">{quotes.map(quote => { const recommended = quote.options.find(option => option.is_recommended) ?? quote.options[0]; const receivedRates = quote.provider_requests.filter(rate => rate.state === "RECEIVED").length; const latestOffer = quote.offers?.[0]; return <div className="quoteInboxEntry" key={quote.id}><article><div className="quoteIdentity"><strong>{quote.quote_number}</strong><span>{quote.company_name || quote.contact_name} · {quote.contact_email}</span><small>{quote.origin_city}, {quote.origin_country} → {quote.destination_city}, {quote.destination_state}</small></div><div><span className="pill warn">{quote.state.replaceAll("_", " ")}</span><small>{quote.pieces} pcs · {Number(quote.cubic_meters).toFixed(2)} CBM · {Number(quote.total_weight_kg).toLocaleString()} kg</small></div><div className="quotePrice"><small>{latestOffer ? `Firm offer v${latestOffer.version}` : recommended ? `${recommended.mode.replaceAll("_", " ")} · ${recommended.arrival_port}` : "Needs rate plan"}</small><strong>{latestOffer ? `$${(latestOffer.total_minor / 100).toLocaleString(undefined,{maximumFractionDigits:0})}` : recommended ? `$${(recommended.total_minor / 100).toLocaleString(undefined,{maximumFractionDigits:0})}` : "—"}</strong><span>{receivedRates ? `${receivedRates} verified provider costs` : "Supplier costs not recorded"}</span></div><div className="quoteDescription"><small>Cargo</small><p>{quote.cargo_description}</p><time>{new Date(quote.requested_at).toLocaleString()}</time><button className="secondary" onClick={() => setSelectedQuoteId(current => current === quote.id ? null : quote.id)}>{selectedQuoteId === quote.id ? "Close pricing" : "Price & publish"}</button></div></article>{selectedQuoteId === quote.id ? <QuoteExecutionPanel quote={quote} onChanged={load} onError={setMessage} /> : null}</div>; })}{!quotes.length ? <p className="emptyState">No quote requests yet. Requests from the customer calculator will appear here immediately.</p> : null}</div></section>
